...
public class WeirdApplicationFactory<TEntryPoint> : IDisposable, IAsyncDisposable where TEntryPoint : class
{
    ...
    private readonly List<WeirdApplicationFactory<TEntryPoint>> _derivedFactories = new();
    private readonly Func<IHostEnvironment?, bool>? _hostFilter;

    /// <summary>
    /// <para>
    /// Creates an instance of <see cref="WeirdApplicationFactory{TEntryPoint}"/>. This factory can be used to
    ...
    /// The <see cref="WeirdApplicationFactory{TEntryPoint}"/> will find the entry point class of <typeparamref name="TEntryPoint"/>
    ...
    /// <see cref="Microsoft.AspNetCore.Mvc.Testing.WebApplicationFactoryContentRootAttribute"/> on the assembly containing the functional tests with
    /// a key equal to the <typeparamref name="TEntryPoint"/> assembly <see cref="Assembly.FullName"/>.
    /// In case an attribute with the right key can't be found, <see cref="WeirdApplicationFactory{TEntryPoint}"/>
    ...
    /// </para>
    /// </summary>
    public WeirdApplicationFactory(Func<IHostEnvironment?, bool>? hostFilter = null)
    {
    ...
    /// <summary>
    /// Finalizes an instance of the <see cref="WeirdApplicationFactory{TEntryPoint}"/> class.
    /// </summary>
    ~WeirdApplicationFactory()
    {
        Dispose(false);
    }

    /// <summary>
    /// Gets the <see cref="TestServer"/> created by this <see cref="WeirdApplicationFactory{TEntryPoint}"/>.
    /// </summary>
    ...
    /// <summary>
    /// Gets the <see cref="IServiceProvider"/> created by the server associated with this <see cref="WeirdApplicationFactory{TEntryPoint}"/>.
    /// </summary>
    ...
    /// <see cref="WeirdApplicationFactory{TEntryPoint}.WithWebHostBuilder(Action{IWebHostBuilder})"/>.
    /// </summary>
    public IReadOnlyList<WeirdApplicationFactory<TEntryPoint>> Factories => _derivedFactories.AsReadOnly();

    /// <summary>
    /// Gets the <see cref="Microsoft.AspNetCore.Mvc.Testing.WebApplicationFactoryClientOptions"/> used by <see cref="CreateClient()"/>.
    /// </summary>
    public WebApplicationFactoryClientOptions ClientOptions { get; private set; } = new WebApplicationFactoryClientOptions();

    /// <summary>
    /// Creates a new <see cref="WeirdApplicationFactory{TEntryPoint}"/> with a <see cref="IWebHostBuilder"/>
    ...
    /// </param>
    /// <returns>A new <see cref="WeirdApplicationFactory{TEntryPoint}"/>.</returns>
    public WeirdApplicationFactory<TEntryPoint> WithWebHostBuilder(Action<IWebHostBuilder> configuration) =>
        WithWebHostBuilderCore(configuration);

    internal virtual WeirdApplicationFactory<TEntryPoint> WithWebHostBuilderCore(Action<IWebHostBuilder> configuration)
    {
        var factory = new DelegatedWeirdApplicationFactory(
            ClientOptions,
            CreateServer,
            CreateHost,
    ...
    private void EnsureServer()
    {
        ...
        if (builder is null)
        {
            ...
            throw new InvalidOperationException("Could not find a host builder method");
        }
        else
        {
    ...
    private static void EnsureDepsFile()
    {
        ...
        {
            throw new InvalidOperationException("The entry point assembly does not have an entry point defined.");
        }
        ...
        if (!depsFile.Exists)
        {
            throw new InvalidOperationException($"The entry point assembly does not have a deps file: {depsFile.FullName}");
        }
    }
    ...
    public HttpClient CreateClient(WebApplicationFactoryClientOptions options) =>
        CreateDefaultClient(options.BaseAddress, options.CreateHandlersAccessor());
    ...
    /// <summary>
    /// Configures <see cref="HttpClient"/> instances created by this <see cref="WeirdApplicationFactory{TEntryPoint}"/>.
    /// </summary>
    ...
    private sealed class DelegatedWeirdApplicationFactory : WeirdApplicationFactory<TEntryPoint>
    {
        ...
        public DelegatedWeirdApplicationFactory(
        ...
        {
            ClientOptions = WebApplicationFactoryClientOptionsAccessors.WebApplicationFactoryClientOptionsAccessor(options);
        ...
        internal override WeirdApplicationFactory<TEntryPoint> WithWebHostBuilderCore(Action<IWebHostBuilder> configuration)
        {
            return new DelegatedWeirdApplicationFactory(
                ClientOptions,
                _createServer,
                _createHost,
