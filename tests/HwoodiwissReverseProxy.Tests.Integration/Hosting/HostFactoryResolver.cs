 ...
 internal sealed class HostFactoryResolver
    {
        ...
        public static Func<string[], object>? ResolveHostFactory(Assembly assembly,
                                                                 ...
                                                                 Func<IHostEnvironment?, bool>? hostFilter = null)
        {
            ...
            return args => new HostingListener(args, assembly.EntryPoint, waitTimeout ?? s_defaultWaitTimeout, stopApplication, configureHostBuilder, entrypointCompleted, hostFilter).CreateHost();
        }
        ...
        private sealed class HostingListener : IObserver<DiagnosticListener>, IObserver<KeyValuePair<string, object?>>
        {
            ...
            public HostingListener(string[] args, MethodInfo entryPoint, TimeSpan waitTimeout, bool stopApplication, Action<object>? configure, Action<Exception?>? entrypointCompleted, Func<IHostEnvironment?, bool>? hostFilter = null)
            {
                _args = args;
            ...
            public void OnNext(KeyValuePair<string, object?> value)
            {
                ...
                if (value.Key == "HostBuilding"
                    && FilterHostBuilding(value.Value!))
                {
                    _configure?.Invoke(value.Value!);
                }

                if (value.Key == "HostBuilt"
                    && FilterHostBuilt(value.Value))
                {
            ...
            private bool FilterHostBuilding(object? hostBuilder)
            {
                if (_hostFilter is not null)
                {
                    return hostBuilder is IHostBuilder builder 
                           && builder.Properties.Values.OfType<WebHostBuilderContext>().FirstOrDefault() is {} ctx
                           && _hostFilter(ctx.HostingEnvironment);
                }

                return true;
            }
            
            private bool FilterHostBuilt(object? value)
            {
                if (_hostFilter is not null)
                {
                    return value is IHost host && _hostFilter(host.GetWebHostService().GetHostingEnvironment());
                }
