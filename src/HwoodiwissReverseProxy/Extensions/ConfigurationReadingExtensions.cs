...
internal static class ConfigurationReadingExtensions
{
    ...
    {
        return configuration[name] is { } value ? double.Parse(value, CultureInfo.InvariantCulture) : null;
    }
    ...
    internal static TimeSpan? ReadTimeSpan(this IConfiguration configuration, string name)
    {
        ...
        return configuration[name] is { } value ? TimeSpan.ParseExact(value, "c", CultureInfo.InvariantCulture) : null;
    }

    internal static Uri? ReadUri(this IConfiguration configuration, string name)
    {
        return configuration[name] is { } value ? new Uri(value) : null;
    }
    ...
    {
        return configuration[name] is { } value && !string.IsNullOrEmpty(value) ? Version.Parse(value + (value.Contains('.') ? "" : ".0")) : null;
    }
