...
/// </summary>
internal sealed partial class ConfigurationConfigProvider : IProxyConfigProvider, IDisposable
{
    ...
    private static RouteConfig CreateRoute(IConfigurationSection section)
    {
        ...
        {
            throw new ArgumentException("The route config format has changed, routes are now objects instead of an array. The route id must be set as the object name, not with the 'RouteId' field.");
        }
    ...
    private static List<Dictionary<string, string>>? CreateTransforms(IConfigurationSection section)
    {
    ...
    private static HttpClientConfig? CreateHttpClientConfig(IConfigurationSection section)
    {
        ...
        if (section.GetSection(nameof(HttpClientConfig.SslProtocols)) is { } sslProtocolsSection)
        {
    ...
    {
        [LoggerMessage(LogLevel.Error, "An exception was thrown from the change notification.")]
        public static partial void ErrorSignalingChange(ILogger logger, Exception exception);
        
        [LoggerMessage(LogLevel.Information, "Loading proxy data from config.")]
        public static partial void LoadData(ILogger logger);
        
        [LoggerMessage(LogLevel.Error, "Configuration data conversion failed.")]
        public static partial void ConfigurationDataConversionFailed(ILogger logger, Exception exception);
    }
}
